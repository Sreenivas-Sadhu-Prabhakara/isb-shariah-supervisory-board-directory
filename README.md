# Shariah Supervisory Board Directory
...
| POST | `/v1/ssb-member-mandate-directory-entry/initiate` | Initiate |
| GET | `/v1/ssb-member-mandate-directory-entry` | Retrieve (list) |
| GET | `/v1/ssb-member-mandate-directory-entry/{crId}/retrieve` | Retrieve |
| PUT | `/v1/ssb-member-mandate-directory-entry/{crId}/update` | Update |
| PUT | `/v1/ssb-member-mandate-directory-entry/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/ssb-member-mandate-directory-entry/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-shariah-supervisory-board-directory:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-shariah-supervisory-board-directory ./helm -n isb-governance
```

Exposed through the platform Gateway at path prefix `/isb-shariah-supervisory-board-directory` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
